 ...
 */
_Readable.prototype.through = function(transformFunc) {
	if(this.isReadableObjectMode()) {
		return this.pipe(new ThroughStream({ objectMode: true }, function(chunk, enc, cb) {
			transformFunc(chunk, cb);
		}));
	} else {
		return this.pipe(new ThroughStream(null, transformFunc));
	}
};
...
_Readable.prototype.throughObj = function(transformFunc) {
	if(this.isReadableObjectMode()) {
		...
		return this.pipe(new ThroughStream({ readableObjectMode: true, writableObjectMode: false }, transformFunc));
	}
};
...
_Readable.prototype.throughData = function(transformFunc) {
	if(this.isReadableObjectMode()) {
		return this.pipe(new ThroughStream({ readableObjectMode: false, writableObjectMode: true }, function(chunk, enc, cb) {
			transformFunc(chunk, cb);
		}));
	} else {
		return this.pipe(new ThroughStream({}, transformFunc));
	}
};
 ...
 */
_Readable.prototype.throughSync = function(transformFunc) {
...
_Readable.prototype.throughObjSync = function(transformFunc) {
	return this.throughObj(function() {
		var args = Array.prototype.slice.call(arguments, 0);
		var cb = (typeof args[1] === 'function') ? args[1] : args[2];
		try {
			cb(null, transformFunc.apply(this, args.slice(0, -1)));
...
_Readable.prototype.throughDataSync = function(transformFunc) {
	return this.throughData(function() {
		var args = Array.prototype.slice.call(arguments, 0);
		var cb = (typeof args[1] === 'function') ? args[1] : args[2];
		try {
			cb(null, transformFunc.apply(this, args.slice(0, -1)));
...
_Readable.prototype.split = function(delimiter) {
	...
	return this.pipe(new SplitStream(delimiter, { readableObjectMode: true,  writableObjectMode: false }));
};
...
_Readable.prototype.batch = function(batchSize) {
	...
	return this.pipe(new BatchStream(batchSize, { obejctMode: true }));
};
...
_Readable.prototype.pluck = function(property) {
	...
	return this.pipe(new PluckStream(property, { objectMode: true }));
};
...
var JoinStream = require('../streams/join-stream');
