...
function RequestStream(requestStream, options) {
	...
	CompoundDuplex.call(this, options);
	if(requestStream.method === 'GET') {
		this._compoundWrapNoWritablePipe = true;
	}
	this.wrap(requestStream);
}
...
RequestStream.prototype._handleErrorInResponse = function(response, error) {
	...
	if(this._requestStreamOptions.readErrorResponse !== false) {
		this._compoundReadable.unpipe(this._wrapOutputPassThrough);
		this._wrapOutputPassThrough.intoString(function(error2, string) {
			if(error2) {
