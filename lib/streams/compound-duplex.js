...
/**
 * Override the duplexPrefinish function to call the flush/push the null value to the internal writable stream
 *
...
CompoundDuplex.prototype._duplexPrefinish = function() {
	...
	if(typeof this._flush === 'function') {
		var self = this;
		// Run the flush function on the _compoundWritable instead of this
		this._flush.call(this._compoundWritable, function(error) {
			if(error) {
				self.emit('error', error);
			}
			self._compoundWritable.end();
		});
	} else {
