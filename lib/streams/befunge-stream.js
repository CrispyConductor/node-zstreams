...
function BefungeSpace(initialValues) {
	...
	this.quad3 = [];	// -x -y
	this.quad4 = [];	// +x -y
	if(initialValues instanceof Buffer) initialValues = initialValues.toString('utf8');
...
function BefungeIP(space) {
	...
	if(space && space.x && space.dx) {
		...
		// Array of fingerprint IDs, the stack head is at the end of the stack
		this.fingerprintStack = other.fingerprintStack.slice(0);
...
BefungeIP.prototype.getSysInfo = function() {
	...
	var info = [
		...
		this.stackStack.length
	];
	for(var i = 0; i < this.stackStack.length; i++) {
		info.push(this.stackStack[i].stack.length);
	}
	info.push(0, 0, 0);
	return info;
};
...
BefungeInterpreter.prototype.run = function() {
	...
	for(;;) {
		...
		if(!this.instructionPointers.length) {
			this.stopInterpreter();
		}
	}
};
...
BefungeInterpreter.prototype.readMoreInput = function() {
	...
	if(this.bfWritableEOFFlag && !this._inputQueue.length) {
	...
	function nextChar() {
		...
		if(inputQueueEntry.isEOF) return null;
