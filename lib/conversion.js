...
function isClassicStream(stream) {
	return !(stream.pipe && (stream.read || (stream.write && stream._writableState)));
}
...
function isRequestStream(stream) {
	return !!(isClassicStream(stream) && stream.headers && stream.method && stream.__isRequestRequest);
}
...
function convertToZStream(stream, options) {
	...
	var origFuncs = {};
	for (var funcName of [ 'pipe', 'unpipe' ]) {
		if (typeof stream[funcName] === 'function') origFuncs[funcName] = stream[funcName];
	}
